...
__all__ = [
    "CoverageDecoderOutput",
    "CoverageDecoderState",
    "CoverageDecoder",
    "FinalCoverageDecoderOutput",
    "tile_batch",
]


class CoverageDecoderState(
    collections.namedtuple("CoverageDecoderState", ("cell_state", "log_probs",
                                                    "finished", "lengths", "coverages"))):
    pass


class CoverageDecoderOutput(
    collections.namedtuple("CoverageDecoderOutput",
                           ("scores", "predicted_ids", "parent_ids"))):
    pass


class FinalCoverageDecoderOutput(
    collections.namedtuple("FinalCoverageDecoderOutput",
    ...
    Args:
      ...
      beam_search_output: An instance of `CoverageDecoderOutput` that describes
...
def _check_maybe(t):
    if isinstance(t, tensor_array_ops.TensorArray):
        raise TypeError(
            "TensorArray state is not supported by CoverageDecoder: %s" % t.name)
    if t.shape.ndims is None:
        raise ValueError(
            "Expected tensor (%s) to have known rank, but ndims == None." % t)


class CoverageDecoder(decoder.Decoder):
    """Coverage sampling decoder."""
    ...
    def __init__(self,
        ...
        self._coverage_score_weight = coverage_score_weight
        self._coverage_penalty_weight = coverage_penalty_weight
        self._source_sequence_length = source_sequence_length
        print("  threshold: %s" % threshold)
        print("  coverage_score_weight: %s" % coverage_score_weight)
        print("  coverage_penalty_weight: %s" % coverage_penalty_weight)
    ...
    def initialize(self, name=None):
        ...
        initial_state = CoverageDecoderState(
            ...
            coverages=array_ops.zeros(
                [self._batch_size, self._beam_width, self._source_sequence_length], dtype=dtypes.float32)
        )
    ...
    def step(self, time, inputs, state, name=None):
        ...
        with ops.name_scope(name, "CoverageDecoderStep", (time, inputs, state)):
            ...
            beam_search_output, beam_search_state = _beam_search_step(
                ...
                coverage_score_weight=self._coverage_score_weight
            )
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
                      beam_width, end_token, length_penalty_weight, threshold, coverage_score_weight):
    ...
    Args:
      ...
      beam_state: Current state of the beam search.
        An instance of `CoverageDecoderState`.
    ...
    # dimensions of coverages and alignments are only matched when batch_size = 1
    next_coverages = _update_coverage(beam_state.coverages, next_cell_state.alignments, previously_finished)
    ...
    next_coverages = _tensor_gather_helper(
        gather_indices=next_beam_ids,  # ids of which beams as next beams
        gather_from=next_coverages,
        batch_size=batch_size,
        range_size=beam_width,
        gather_shape=[batch_size * beam_width, -1])  # preserve source_sequence_length

    next_state = CoverageDecoderState(
...
def _update_coverage(coverages, alignments, finished):
    """update coverage of j-th word at i-th time step based on c_{ij}=\min(\sum_{t=0}^{T=i} alignments_j, 1.0)"""
    mask_alignments = alignments * tf.expand_dims(tf.to_float(1. - tf.to_float(finished)), axis=-1)
    next_coverages = coverages + mask_alignments
    return next_coverages


def _get_scores_v2(log_probs, coverages, threshold, coverage_score_weight, sequence_lengths, length_penalty_weight):
    vocab_size = tf.shape(log_probs)[-1]
    # 1 coverage scores
    coverage_scores = tf.reduce_sum(tf.log(tf.maximum(coverages, threshold)), axis=-1)
    # 2 coverage penalty
    # coverage_scores = tf.reduce_sum(tf.log(tf.minimum(coverages, 1.)), axis=-1)
    coverage_scores = tf.expand_dims(coverage_scores, axis=-1)
    coverage_scores = tf.tile(coverage_scores, [1, 1, vocab_size])
    with tf.control_dependencies([
        tf.Assert(tf.equal(tf.reduce_sum(tf.to_int32(tf.is_inf(coverage_scores))), 0), [coverage_scores]),
        tf.Assert(tf.equal(tf.reduce_sum(tf.to_int32(tf.is_nan(coverage_scores))), 0), [coverage_scores])
    ]):
        if length_penalty_weight != 0.0:
            log_probs = log_probs / _length_penalty(
                sequence_lengths=sequence_lengths, penalty_factor=length_penalty_weight)
            # 1 normalized by |x|
            #coverage_scores = coverage_scores / tf.to_float(tf.shape(coverages)[-1])
            # 2 normalized by |y|
            coverage_scores = coverage_scores / _length_penalty(
                sequence_lengths=sequence_lengths, penalty_factor=length_penalty_weight)
        scores = (1. - coverage_score_weight) * log_probs + coverage_score_weight * coverage_scores
    return scores
