...
tf.app.flags.DEFINE_boolean('pretrain_embedding', True, 'Use pretrained word embedding')
tf.app.flags.DEFINE_boolean('finetuning', True, 'Fine tuning word embedding')
tf.app.flags.DEFINE_integer('keep_prev_num', 2, 'Number of keep previous sentences')
tf.app.flags.DEFINE_integer('sen_pad_len', 20, 'Number of sentence pad length')
tf.app.flags.DEFINE_integer('keyword_pad_len', 4, 'Number of keyword pad length')
tf.app.flags.DEFINE_string('cell_type', 'lstm', 'RNN cell for encoder and decoder: (gru, lstm), default: lstm')
...
tf.app.flags.DEFINE_integer('vocab_size', get_vocab_size(), 'General vocabulary size')
...
tf.app.flags.DEFINE_integer('max_epochs', 3, 'Maximum # of training epochs')
tf.app.flags.DEFINE_integer('max_load_batches', 20, 'Maximum # of batches to load at one time')
tf.app.flags.DEFINE_integer('max_seq_length', 40, 'Maximum sequence length')
...
def train():
    ...
    with tf.Session(config=config_proto) as sess:
        ...
        # Initialize global variables or reload existing checkpoint
        load_or_create_model(sess, model, saver, FLAGS)

        # initialize embedding with random values
        embedding = uniform(-1.0, 1.0, [FLAGS.vocab_size, FLAGS.hidden_units])

        # Load word2vec embedding, if pre-train
        if FLAGS.pretrain_embedding:
        ...
        for epoch_idx in xrange(FLAGS.max_epochs):
            if model.global_epoch_step.eval() >= FLAGS.max_epochs:
                print 'Training is already complete.', 'Current epoch: {}, Max epoch: {}'.format(
                    model.global_epoch_step.eval(), FLAGS.max_epochs)
                break

            # Prepare batch training data
            for source, source_len, target, target_len in gen_batch_train_data(FLAGS.batch_size, prev=FLAGS.prev_data,
                                                                               rev=FLAGS.rev_data,
                ...
                sents_seen += float(source.shape[0])
                ...
                if model.global_step.eval() % FLAGS.display_freq == 0:
                    ...
                    print 'Epoch ', model.global_epoch_step.eval(), 'Step ', model.global_step.eval(), 'Perplexity {0:.2f}'.format(
                        avg_perplexity), 'Step-time ', step_time, '{0:.2f} sents/s'.format(sents_per_sec)
                ...
                if model.global_step.eval() % FLAGS.save_freq == 0:
                    ...
                    json.dump(model.config, open('%s-%d.json' % (checkpoint_path, model.global_step.eval()), 'wb'),
                              indent=2)
        ...
        checkpoint_path = os.path.join(FLAGS.model_dir, FLAGS.model_name)
        model.save(sess, saver, checkpoint_path, global_step=model.global_step)
        json.dump(model.config, open('%s-%d.json' % (checkpoint_path, model.global_step.eval()), 'wb'), indent=2)
