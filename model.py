...
class Seq2SeqModel:
    ...
    def __init__(self, config, mode):
        """For tensorflow 1.6 and above, Flag type is independant from default types.
        The values can be retrieved by flag.value instead."""
    ...
    def init_placeholders(self):
        ...
        self.embedding_placeholder = tf.placeholder(name='embedding_placeholder',
            shape=[self.vocab_size, self.hidden_units], dtype=self.dtype)

        self.embedding = tf.get_variable(name='embedding', shape=[self.vocab_size, self.hidden_units],
            trainable=self.finetuning, dtype=self.dtype)

        self.assign_embedding_op = self.embedding.assign(self.embedding_placeholder)

        # encode_inputs: [batch_size, time_steps]
        self.encoder_inputs = tf.placeholder(name='encoder_inputs', shape=(None, None), dtype=tf.int32)

        # Total length of each input sequence.
        # encoder_inputs_length: [batch_size]
        self.encoder_inputs_length = tf.placeholder(name='encoder_inputs_length', shape=(None,), dtype=tf.int32)
        ...
        if self.mode == 'train':
            ...
            # Start and end tokens are initiated as one vectors
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            self.encoder_inputs_embedded = tf.nn.embedding_lookup(params=self.embedding, ids=self.encoder_inputs)
            ...
            if self.bidirectional:
                ...
                self.encoder_outputs_fw_bw, self.encoder_last_state_fw_bw = tf.nn.bidirectional_dynamic_rnn(
                    cell_fw=self.encoder_cell_fw, cell_bw=self.encoder_cell_bw, inputs=self.encoder_inputs_embedded,
                ...
                encoder_last_state_list = [
                    rnn.LSTMStateTuple(c=tf.concat([fw.c, bw.c], 1), h=tf.concat([fw.h, bw.h], 1)) for fw, bw in
                    encoder_last_state_zipped]
                ...
                self.encoder_outputs, self.encoder_last_state = tf.nn.dynamic_rnn(cell=self.encoder_cell,
                    inputs=self.encoder_inputs_embedded, sequence_length=self.encoder_inputs_length, dtype=self.dtype,
                    time_major=False)

    def build_decoder_cell(self):

        if self.use_beamsearch_decode:
            encoder_outputs = tf.contrib.seq2seq.tile_batch(self.encoder_outputs, multiplier=self.beam_width)
            encoder_last_state = tf.contrib.seq2seq.tile_batch(self.encoder_last_state, multiplier=self.beam_width)
        ...
        self.attention_mechanism = seq2seq.BahdanauAttention(num_units=self.decoder_hidden_units,
        ...
        def attn_decoder_input_fn(inputs, attention):
            ...
            _input_layer = Dense(self.decoder_hidden_units, dtype=self.dtype, name='attn_input_feeding')
            return _input_layer(rnn.array_ops.concat([inputs, attention], -1))

        # Attention mechanism is implemented only on all decoder layers
        self.decoder_cell_list = seq2seq.AttentionWrapper(cell=rnn.MultiRNNCell(self.decoder_cell_list),
            attention_mechanism=self.attention_mechanism, attention_layer_size=self.decoder_hidden_units,
            cell_input_fn=attn_decoder_input_fn, initial_cell_state=encoder_last_state, alignment_history=False,
        ...
        # add by Meng
        decoder_initial_state = self.decoder_cell_list.zero_state(batch_size=batch_size, dtype=self.dtype).clone(
            cell_state=encoder_last_state)

        return self.decoder_cell_list, decoder_initial_state

    def build_train_decoder(self):
        self.decoder_inputs_embedded = tf.nn.embedding_lookup(params=self.embedding, ids=self.decoder_inputs_train)
        if self.train_mode == 'ground_truth':
            training_helper = seq2seq.TrainingHelper(inputs=self.decoder_inputs_embedded,
                sequence_length=self.decoder_inputs_length_train, time_major=False, name='training_helper')
        elif self.train_mode == 'scheduled_sampling':
            training_helper = seq2seq.ScheduledEmbeddingTrainingHelper(inputs=self.decoder_inputs_embedded,
                sequence_length=self.decoder_inputs_length_train,
                embedding=lambda inputs: tf.nn.embedding_lookup(self.embedding, inputs),
                sampling_probability=self.sampling_probability, name='scheduled_embedding_training_helper')
        else:
        ...
        self.decoder_outputs_train, self.decoder_last_state_train, self.decoder_outputs_length_train = seq2seq.dynamic_decode(
            decoder=training_decoder, output_time_major=False, impute_finished=True,
        ...
        masks = tf.sequence_mask(lengths=self.decoder_inputs_length_train, maxlen=max_decoder_length, dtype=self.dtype,
            name='masks')

        # Computes per word average cross-entropy over a batch
        # Internally calls 'nn_ops.sparse_softmax_cross_entropy_with_logits' by default
        self.loss = seq2seq.sequence_loss(logits=self.decoder_logits_train, targets=self.decoder_targets_train,
            weights=masks, average_across_timesteps=True, average_across_batch=True)
    ...
    def build_predict_decoder(self):
        ...
        if not self.use_beamsearch_decode:
            ...
            if self.predict_mode == 'sample':
                ...
                decoding_helper = seq2seq.SampleEmbeddingHelper(start_tokens=start_tokens, end_token=end_token,
                    embedding=lambda inputs: tf.nn.embedding_lookup(self.embedding, inputs))
            elif self.predict_mode == 'greedy':
                print 'Building greedy decoder...'
                decoding_helper = seq2seq.GreedyEmbeddingHelper(start_tokens=start_tokens, end_token=end_token,
                    embedding=lambda inputs: tf.nn.embedding_lookup(self.embedding, inputs))
            else:
                raise NotImplementedError('Predict mode: {} is not yet implemented'.format(self.predict_mode))

            inference_decoder = seq2seq.BasicDecoder(cell=self.decoder_cell, helper=decoding_helper,
                initial_state=self.decoder_initial_state, output_layer=self.output_layer)
        else:
            # Beam search decoder
            ...
            inference_decoder = seq2seq.CoverageDecoder(cell=self.decoder_cell,
                ...
                output_layer=self.output_layer, threshold=0.3, coverage_score_weight=0.6, coverage_penalty_weight=0,
        ...
        if not self.use_beamsearch_decode:
            ...
            self.decoder_pred_decode = tf.expand_dims(self.decoder_outputs_decode.sample_id, -1, name='out_put0000')
        else:
            # added by Meng
            # self.decoder_pred_decode = self.decoder_outputs_decode.predicted_ids
            self.decoder_pred_decode = tf.identity(self.decoder_outputs_decode.predicted_ids,
                                                   name='out_put0000')
    ...
    def predict(self, sess, encoder_inputs, encoder_inputs_length):
        input_feed = self.check_feeds(encoder_inputs, encoder_inputs_length, decoder_inputs=None,
                                      decoder_inputs_length=None, predict=True)
    ...
    def init_vars(self, sess, embedding):
        sess.run([self.assign_embedding_op], feed_dict={self.embedding_placeholder: embedding})

    def check_feeds(self, encoder_inputs, encoder_inputs_length, decoder_inputs, decoder_inputs_length, predict):
        """
        Args:
          encoder_inputs: a numpy int matrix of [batch_size, max_source_time_steps]
              to feed as encoder inputs
